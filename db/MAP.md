...
| `apply-projects.js` | Re-runnable step that resolves `projects.json` member subjects to current topic ids and writes `projects` + `project_topics` (ADR 0010, same pattern as apply-relations). `--self-test` / `--dry-run` / `--apply` / `--rebuild`. |
| `projects.json` | Source of truth for Projects (ADR 0010) — human-confirmed groupings, subject-keyed, human-chosen slug ids. Empty until the first seeding pass (#93). |
...
| `migrations/` | Applied SQL migrations 0001–0009 — see `migrations/MAP.md`. |
